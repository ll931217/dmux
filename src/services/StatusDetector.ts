      ...
      if (error.message) {
        ...
        if (error.message.includes('API error')) {
          ...
          if (match) {
            ...
            if (status === '401') {
              errorMessage = `API auth failed - check your AI provider API key`;
            } else if (status === '402') {
          ...
          errorMessage = 'AI provider not configured - run dmux setup';
        } else if (error.message.includes('All models')) {
          errorMessage = 'AI provider error - check API key & credits';
